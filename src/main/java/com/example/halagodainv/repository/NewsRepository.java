...
public interface NewsRepository extends JpaRepository<NewsEntity, Integer> {
    ...
    List<NewDto> getNewList(@Param("title") String title, @Param("topicId") Long topicId, @Param("tagId") String tagId, Pageable pageable);
    ...
    @Query(value = "select count(n) from NewsEntity n left join NewsLanguageEntity nl " +
            ...
            "and n.tagId like concat('%',:tagId,'%') " +
            "and (:topicId is null or :topicId = 0l or n.topicId =:topicId ) ")
    int countByAll(@Param("title") String title, @Param("topicId") Long topicId, @Param("tagId") String tagId);


    @Query("select new com.example.halagodainv.dto.news.NewDetails(n.idNews,nl.title,n.thumbnail, " +
            "nl.description,nl.content,c.id,n.status,n.titleSeo,n.linkPapers,nl.language,n.topicId,n.tagId,n.isHot,n.isVew) " +
            "from NewsEntity n left join NewsLanguageEntity nl " +
            "on n.idNews = nl.newsEntity.idNews " +
    ...
    @Query("select new com.example.halagodainv.dto.viewnews.ViewNewsMap(n.idNews,nl.title,nl.content,n.created,n.topicId,n.tagId,n.thumbnail) from  NewsEntity n " +
            ...
            "where (:#{#topicId} = 0l or n.topicId=:#{#topicId}) and n.tagId like concat('%',:#{#tagId},'%') and nl.language =:#{#language} and n.isVew =true ")
    ...
    @Query("select new com.example.halagodainv.dto.viewnews.ViewNewsMap(n.idNews,nl.title,nl.content,n.created,n.topicId,n.tagId,n.thumbnail) from  NewsEntity n " +
            "left join NewsLanguageEntity nl on n.idNews = nl.newsEntity.idNews " +
            "where (:#{#topicId} = 0l or n.topicId=:#{#topicId}) and n.tagId like concat('%',:#{#tagId},'%') and nl.language =:#{#language} and n.idNews =:#{#id} and n.isVew = true")
    ...
    @Query("select new com.example.halagodainv.dto.viewnews.ViewNewAndHot(n.idNews,nl.title,n.thumbnail) from  NewsEntity n " +
            "left join NewsLanguageEntity nl on n.idNews = nl.newsEntity.idNews " +
            "where (:#{#topicId} = 0l or n.topicId=:#{#topicId}) and n.tagId like concat('%',:#{#tagId},'%') and nl.language =:#{#language} and n.topicId <> 1 and n.topicId <> 6  and n.isVew =true ")
    ...
    @Query("select new com.example.halagodainv.dto.viewnews.ViewNewAndHot(n.idNews,nl.title,n.thumbnail) from  NewsEntity n " +
            "left join NewsLanguageEntity nl on n.idNews = nl.newsEntity.idNews " +
            "where (:#{#topicId} = 0l or n.topicId=:#{#topicId}) and n.tagId like concat('%',:#{#tagId},'%') and nl.language =:#{#language} and n.topicId <> 1 and n.topicId <> 6 and n.isHot =true and n.isVew =true ")
